...
fn main() -> Result<(), BoxError> {
    ...
    println!("cargo:rerun-if-changed=build/accessors.c");
    ...
    // Bitfield accessors that have to be performed by the C compiler; see build/accessors.c
    compile_accessors(&nginx);
    Ok(())
}
...
/// Compile the C accessors in `build/accessors.c`.
///
/// bindgen derives bitfield positions on its own instead of asking the C
/// compiler, and the result does not match the AArch64 rule that a bitfield
/// may not straddle its container. Reading `header_only` through the
/// generated accessor therefore returns the wrong bit there. Going through
/// the C compiler avoids having to model any ABI's packing rules.
fn compile_accessors(nginx: &NginxSource) {
    let autoconf_makefile_path = nginx.build_dir.join("Makefile");
    let (includes, defines) = parse_makefile(&autoconf_makefile_path);
    let includes: Vec<_> = includes
        .into_iter()
    ...
    builder.includes(&includes).file("build/accessors.c");

    for (name, value) in &defines {
        builder.define(name, value.as_deref());
    }

    if cfg!(feature = "http") {
        builder.define("NGX_RS_FEATURE_HTTP", "1");
    }

    builder.compile("ngx_rs_accessors");
}
